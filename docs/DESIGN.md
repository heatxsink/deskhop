...
**Path A in progress.** Phase 1's hand-rolled gesture state machine produces good basic behavior but every "missing" feature (tap-to-click, tap-and-drag, drag-lock, double-tap, palm rejection) is a new corner case to design and debug. Rather than invent these one by one, the plan is to faithfully port libinput's `evdev-mt-touchpad-tap.c` state machine into the firmware. We inherit ten years of tested edge-case decisions and stop hand-rolling each gesture. Gated behind `DH_TRACKPAD_TAP_TO_CLICK` (OFF by default) so default builds aren't affected by flash growth or behavior change while the port is in development.
